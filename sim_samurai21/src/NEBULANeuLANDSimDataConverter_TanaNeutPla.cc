...
int NEBULANeuLANDSimDataConverter_TanaNeutPla::ConvertSimData()
{
  ...
  for (int ihit=0;ihit<nhit_nebula;++ihit){
    ...
    if (it==fDataBufferMap.end()) {
      ...
      data.primary_id = simdata->fPrimaryParticleID;
      fDataBufferMap.insert(std::pair<int,tmp_data>(data.id,data));

      tmp_data* datap = &(fDataBufferMap[data.id]);
      ///
      itp = fDataBufferMap_TrackID.find(datap->primary_id);
      if (itp == fDataBufferMap_TrackID.end()){
	datap->isprimary = true;
	fDataBufferMap_TrackID.insert(std::pair<int,tmp_data*>(datap->primary_id,datap));
      }else {
	tmp_data *ddd = itp->second;
	if (ddd->t > datap->t){
	  ddd->isprimary = false;
	  datap->isprimary = true;
	  fDataBufferMap_TrackID[data.primary_id] = datap;
	}
      }
      ///
	...
	data->t   = simdata->fPreTime;
	data->pos = simdata->fPrePosition;
	if (simdata->fPrimaryParticleID != data->primary_id)
	  data->primary_id = simdata->fPrimaryParticleID;

	itp = fDataBufferMap_TrackID.find(data->primary_id);
	if (itp == fDataBufferMap_TrackID.end()){
	  data->isprimary = true;
	  fDataBufferMap_TrackID.insert(std::pair<int,tmp_data*>(data->primary_id,data));
	}else {
	  tmp_data *ddd = itp->second;
	  if (ddd->t > data->t){
	    ddd->isprimary = false;
	    data->isprimary = true;
	    fDataBufferMap_TrackID[data->primary_id] = data;
	  }
	}
	///
      }
  ...
  while (it!=fDataBufferMap.end()){
	...
	pla = new ((*fVetoPlaArray)[npla]) TanaNeutPla();
      ...
      pla->SetTrackID(data->primary_id);
