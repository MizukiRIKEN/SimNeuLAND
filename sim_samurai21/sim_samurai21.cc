...
int main(int argc,char** argv)
{
  ...
  SAMURAI21DetectorConstruction* detector = new SAMURAI21DetectorConstruction();
  runManager->SetUserInitialization(detector);
//  runManager->SetUserInitialization(new HODConstruction);
  ...
  simDataManager->RegistInitializer(new NeuLANDSimDataInitializer);
#if defined(TANA)
  simDataManager->FindInitializer("FragSimDataInitializer")->SetDataStore(false);
  simDataManager->RegistConverter(new SAMURAI21BeamSimDataConverter);
  simDataManager->RegistConverter(new SAMURAI21FragSimDataConverter);
  simDataManager->RegistConverter(new NEBULANeuLANDSimDataConverter_TanaNeutPla);
//  NEBULANeuLANDSimDataConverter_TanaNeutPla* converter=  new NEBULANeuLANDSimDataConverter_TanaNeutPla;  
//  converter->SetIncludeResolution(false);
//  simDataManager->RegistConverter(converter);
#else
  simDataManager->RegistConverter(new NEBULANeuLANDSimDataConverter_TArtNEBULAPla);
#endif

  simDataManager->PrintInitializers();
  simDataManager->PrintConverters();
